...
import { adminApi, employeeApi } from '../../api/roles'
...
export default function AdminTimeLogs() {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs.filter((l) => {
      ...
      const hrs = Number(durationHrs(l.start_time, l.end_time))
  ...
  // Org-wide picker for "+ Log Time" - unlike the manager page (scoped to
  // their team), admin can log on behalf of anyone.
  // const teamMembers = users.filter((u) => u.isAlive).map((u) => ({ id: u.id, name: u.name }))
  ...
  async function handleSave(payload: any) {
    if (editing) await adminApi.updateAnyTimeLog(editing.id, payload)
    else if (payload.user_id) await adminApi.logTimeFor(payload)
  ...
  async function confirmDelete() {
    ...
    setDeleting(true)
    try {
      await adminApi.deleteAnyTimeLog(deleteTarget.id)
      load()
    } catch (err) {
  ...
  async function handleApprove(id: number) {
    setBusyId(id)
    try {
      await adminApi.approveTimeLog(id)
      load()
    } catch (err) {
  ...
  async function confirmReject(reason: string) {
    ...
    try {
      await adminApi.rejectTimeLog(rejectTarget.id, reason)
      load()
  ...
  return (
    <div>
      <PageHeader
        title="Time Logs"
        subtitle="Every logged entry across the organization"
        action={
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-3">
            ...
            <div className="flex items-center gap-2 overflow-x-auto">
              ...
              <button type="button" className="btn-ghost text-xs whitespace-nowrap" onClick={() => {
              ...
              {hasActiveFilters && (
                <button type="button" className="btn-ghost text-xs whitespace-nowrap" onClick={clearFilters}>
                  Clear all
                </button>
              )}
            </div>
          ...
          <div className="flex items-end gap-3 overflow-x-auto pb-1">
            <div className="min-w-[160px] flex-1">
              ...
              <select className="input w-full" value={filters.employee} onChange={(e) => setFilter('employee', e.target.value)}>
            ...
            <div className="min-w-[160px] flex-1">
              ...
              <select className="input w-full" value={filters.manager} onChange={(e) => setFilter('manager', e.target.value)}>
            ...
            <div className="min-w-[130px]">
              ...
              <select className="input w-full" value={filters.type} onChange={(e) => setFilter('type', e.target.value)}>
            ...
            <div className="min-w-[145px]">
              ...
              <input type="date" className="input w-full" value={filters.dateFrom}
            ...
            <div className="min-w-[145px]">
              ...
              <input type="date" className="input w-full" value={filters.dateTo}
            ...
            <div className="min-w-[120px]">
              ...
              <input type="number" min="0" step="0.25" className="input w-full"
                value={filters.hoursMin} onChange={(e) => setFilter('hoursMin', e.target.value)} placeholder="Min" />
            </div>

            <div className="min-w-[120px]">
              <label className="text-xs text-muted mb-1 block">Max hours</label>
              <input type="number" min="0" step="0.25" className="input w-full"
                value={filters.hoursMax} onChange={(e) => setFilter('hoursMax', e.target.value)} placeholder="Max" />
            </div>
          </div>
        </div>
      </div>
      ...
      <TimeLogFormModal
        ...
        projects={projects}
        fetchTasks={(projectId) => adminApi.listTasks(projectId)}
        editing={editing}
