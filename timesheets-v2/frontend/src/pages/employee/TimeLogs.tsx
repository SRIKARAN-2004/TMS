...
export default function EmployeeTimeLogs() {
  ...
  function load() {
    setLoading(true)
    Promise.all([employeeApi.myTimeLogs(), employeeApi.myProjects()])
      .then(([l, p]) => {
        setLogs(l)
        setProjects(p)
      })
  ...
  async function handleSave(payload: any) {
    if (editing) await employeeApi.updateTimeLog(editing.id, payload)
  ...
  async function confirmDelete() {
    ...
    setDeleting(true)
    try {
      await employeeApi.deleteTimeLog(deleteTarget.id)
      load()
    } catch (err) {
  ...
  return (
    <div>
              ...
              <tr key={l.id} className="border-b border-border last:border-0 hover:bg-slate-50">
                ...
                <td className="px-5 py-3 text-right">
                  <div className="flex justify-end gap-1">
                    ...
                    {l.status === 'pending' && (
                      <>
                        <IconAction icon={<Icon.Pencil />} label="Edit" onClick={() => openEdit(l)} />
                        <IconAction icon={<Icon.Trash />} label="Delete" variant="danger" onClick={() => handleDelete(l.id)} />
                      </>
                    )}
                  </div>
                </td>
      ...
      <TimeLogFormModal
        ...
        projects={projects}
        fetchTasks={(projectId) => employeeApi.tasksForProject(projectId)}
        editing={editing}
      />

      <TimeLogViewModal open={viewOpen} onClose={() => setViewOpen(false)} log={viewTarget} showEmployee={false} />
